...
{
	//point light
	float4 pointLightPosition = {-5.0f, 5.0f, -5.0f, 1.0f};
	pointLightPosition.xz *= (sin(t.w * 5));
	//pointLightPosition *= cos(time.w);
	float4 pointLightColor = { 0, 1, 0, 1 };
	...
	float4 RESULT = ((LIGHTRATIO * lightClr) + (pointLight*(pointAttenuation*pointAttenuation))+ ((spotFactor * spotLightRatio * spotLightColor) * (spotAttenuation))) * baseTexture.Sample(filter, uvs);
	return RESULT;
}